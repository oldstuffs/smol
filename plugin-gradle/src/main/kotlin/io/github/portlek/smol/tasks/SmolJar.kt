...
import io.github.portlek.smol.resolver.strategy.*
import java.io.File
...
constructor(private val smolConfig: Configuration, private val smolApiConfig: Configuration) :
  ...
  var outputDirectory = File("${project.buildDir}/resources/smol/")
    @OutputDirectory get
  ...
  internal fun generateResolvedDependenciesFile() =
      with(project) {
        ...
        val mapType = object : TypeToken<MutableMap<String, ResolutionResult>>() {}.rawType()
        val preResolved =
            if (file.exists()) {
        ...
        val checksumResolutionStrategy =
            MavenChecksumPathResolutionStrategy("SHA-1", resolutionStrategy)
        val urlPinger = HttpURLPinger()
        val enquirerFactory =
            PingingRepositoryEnquirerFactory(
                resolutionStrategy, checksumResolutionStrategy, pomURLCreationStrategy, urlPinger)
        val mirrorSelector = SimpleMirrorSelector()
        val resolver =
            CachingDependencyResolver(
                urlPinger, mirrorSelector.select(repositories, mirrors), enquirerFactory, mapOf())
        val result =
            runBlocking(Dispatchers.IO) {
              dependencies
                  .filter {
                    preResolved[it.dependencyNotation()]?.let { pre ->
                      repositories.none { r ->
                  ...
                  .map {
                    scope.async { it.dependencyNotation() to resolver.resolve(it).orElse(null) }
                  }
                  .associate { it.await() }
        ...
        FileWriter(file).use { gson.toJson(result, it) }
        if (shadowWriteFolder.exists().not()) {
          shadowWriteFolder.mkdirs()
        }
        file.copyTo(File(shadowWriteFolder, file.name), true)
